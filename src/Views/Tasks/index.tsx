...
const Tasks = () => {
    const tasksData = useSelector((state: ITask[]) => { return state });
    ...
    const changeStatus = (record: ITask) => {
        const newData = [...tasks];
        newData.find(item => item.key === record?.key).completed = !record?.completed;
    ...
    const columns: ColumnsType<ITask> = [
        ...
        },
        {
            ...
            key: 'action',
            render: (_, record) => (
                <a onClick={() => changeStatus(record)}>{record?.completed ? "undone" : "done"}</a>
            ),
        },
    ];
